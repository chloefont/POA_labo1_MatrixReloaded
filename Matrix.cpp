...
Matrix::Matrix(size_t nbRows, size_t nbCols, unsigned modulo, unsigned** matrix) :
   nbRows(nbRows), nbCols(nbCols), modulo(modulo) {
   if (matrix == nullptr)
   ...
   for (size_t i = 0; i < nbRows; i++) {
         memcpy(matrix[i], matrix[i], nbCols * sizeof(unsigned));
   }
}
